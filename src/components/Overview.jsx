...
const Overview = () => {
  const { accounts, transactions, card, } = MOCK_DATA;
  const [showBalance, setShowBalance] = useState(true);

  // 1. Dynamic Calculations (keeping everything in USD)
  const totalBalance = accounts.reduce((sum, acc) => sum + acc.balance, 0);
  const totalIncome = 7200.0;
  const totalExpenses = 3150.0;
  const spendingLimit = 5000.0;
  const limitPercentage = Math.round((totalExpenses / spendingLimit) * 100);
...
<div className="relative">
    <div className="absolute md:hidden sticky top-0 left-0 w-full z-50 bg-slate-900/95 backdrop-blur-lg">
        <Mobileheader />
      </div>

    <div className="md:space-y-8 pb-10">
      {/* HEADER */}


      <section>
        <p className="hidden md:block text-slate-400 text-sm">
          <Greeting /> Here is your account status.
        </p>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        ...
        <div className="bg-slate-800/40 border border-slate-800 p-8 rounded-[2rem] relative overflow-hidden group flex flex-col justify-between">
          {/* Hero Glows */}
          ...
          <div className="relative z-10 space-y-12">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <Link className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl transition-all active:scale-[0.97] shadow-xl shadow-indigo-600/20 group/btn" to={"/transfer"}> 
        ...
        <Link to="analys">
        <div className="bg-slate-800/40 border border-slate-800 p-8 rounded-[2rem] relative overflow-hidden flex flex-col justify-between group">
          {/* Analytics Glows */}
                ...
                <div className="relative w-24 h-24 flex items-center justify-center">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeDashoffset={251.2 - (251.2 * limitPercentage) / 100}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
        ...
        <div className="flex flex-col gap-6">
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-violet-800 p-6 rounded-[2rem] shadow-2xl aspect-[1.58/1] flex flex-col justify-between text-white relative overflow-hidden group cursor-pointer transition-transform active:scale-95">
              ...
              <div className="flex justify-between items-start relative z-10">
                <Wallet size={24} />
                <span className="italic font-black text-sm tracking-tighter">
                  VISA
                </span>
              </div>
          ...
          <div className="bg-slate-800/40 border border-slate-800 p-6 rounded-[2rem] flex-1 group">
            <h3 className="text-sm font-semibold text-white mb-4">
              Quick Transfer
            </h3>
            <div className="grid grid-cols-4 gap-3">
              {["JD", "AS", "MK", "LR"].map((initial, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center gap-2 cursor-pointer group/item"
                >
                  <div className="h-10 w-10 rounded-full bg-slate-700 border border-slate-600 flex items-center justify-center text-[10px] font-bold text-slate-300 group-hover/item:border-indigo-500 group-hover/item:text-white transition-all">
                    {initial}
                  </div>
                  <p className="text-[10px] text-slate-500 group-hover/item:text-slate-300 transition-colors">
                    Contact
                  </p>
                </div>
              ))}
        ...
        <div className="lg:col-span-2 flex flex-col space-y-4">
          ...
          <div className="bg-slate-800/40 border border-slate-800 rounded-[2rem] overflow-hidden divide-y divide-slate-800 flex-1 flex flex-col">
            {transactions.slice(0, 7).map((tx) => (
              <div
                key={tx.id}
                className="flex items-center justify-between p-4 hover:bg-slate-800/60 transition-colors flex-1 min-h-[70px]"
              >
