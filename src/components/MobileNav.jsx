...
const MobileNav = () => {
 ...
 const navLinks = [
  ...
  return ( 
    <nav className="fixed bottom-0 left-0 w-full bg-slate-900/95 backdrop-blur-lg border-t border-white/5 px-2 pb-safe-area pt-3 flex justify-around items-end z-50">
      {navLinks.map((item) => {
        ...
        return (
          <Link 
            key={item.name} 
            to={item.path} 
            className={`flex flex-col items-center gap-1 min-w-[64px] transition-all duration-200 active:scale-90 ${
              isActive ? "text-bank-yellow" : "text-gray-500"
            }`}
          >
            {/* Icon Wrapper with a slight bounce effect if active */}
